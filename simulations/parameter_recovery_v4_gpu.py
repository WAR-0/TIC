#!/usr/bin/env python3
"""
TIC Parameter Recovery v4 (GPU-Ready)
====================================

This script mirrors the definitive v4 simulation but reimplements the estimator
with JAX so it can run efficiently on GPU hardware (e.g., your RTX 4080 rig).
It keeps all hardened conditions, the hierarchical D₀, and literature-backed
priors. Use it locally on the high-powered machine while the CPU-only run
continues elsewhere.

Quick Start on the 4080 Machine
-------------------------------
1. Clone your TIC repository onto the workstation.
2. Create and activate a fresh virtual environment (Python 3.10+ recommended).
...
3. Install JAX (CUDA build) and Optax. Adjust the CUDA version to match your
   drivers; the example below targets CUDA 12.x which fits recent RTX GPUs.
   ...
   *If the above link doesn’t match your CUDA toolchain, consult the JAX
   installation docs: https://github.com/google/jax#installation.*

4. Run the GPU-ready simulation (adjust counts as needed):

   ````bash
   python simulations/parameter_recovery_v4_gpu.py \
       --n-sim 500 --n-participants 35 --max-steps 1500
   ````

   The script will auto-detect GPU availability and place computations there.
   Summary stats land in `simulations/results_v4_gpu.txt`.

Implementation highlights
------------------------
* Hardened data generation identical to v4 (calibration block with zero lapses,
  Φ'-independent D_eff, novelty-density decoupling, Student-t noise).
* Hierarchical estimation with one shared D₀; per-participant λ, κ, γ.
* Priors-as-penalties reflecting cognitive anchors.
* Objective and gradients computed with JAX, batched across participants and
  trials; optimized with optax.Adam, jitted for GPU/TPU execution.

Note: JAX’s first compilation per data shape incurs a one-time cost. After the
first simulation, subsequent runs reuse the compiled graph and fly on GPU.
"""
...
class SimulationBatch:
    D_eff: jnp.ndarray      # shape (n_participants, n_trials)
    N_obs: jnp.ndarray      # shape (n_participants, n_trials)
    Phi: jnp.ndarray        # shape (n_participants, n_trials)
    Ts: jnp.ndarray         # shape (n_participants, n_trials)
...
class TICParameterRecoveryV4GPU:
    ...
    def generate_single_dataset(self) -> Tuple[SimulationBatch, Dict[str, np.ndarray]]:
        ...
        batch = SimulationBatch(
            D_eff=jnp.array(D_eff_mat),
            N_obs=jnp.array(N_obs_mat),
            Phi=jnp.array(Phi_mat),
            Ts=jnp.array(Ts_mat),
        )
        true_params = {
            "D0": np.array(D0_true),
    ...
    def build_loss_fn(self) -> Tuple[int, callable]:
        ...
        def loss_fn(params: jnp.ndarray, data: SimulationBatch) -> jnp.ndarray:
