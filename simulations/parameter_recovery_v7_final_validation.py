#!/usr/bin/env python3
"""
Parameter Recovery Simulation for TIC Model v7 (Final Validation)

This version locks in the hierarchical PyMC/NUTS model from v6 but upgrades the
experimental design to the "Bookend" layout required for full identifiability:

1. Low-Entropy Anchor block (15 trials, density≈0.05, novelty≈0.05) with reduced
   noise and zero lapses to cleanly estimate D₀.
2. Factorial Cross block (3 densities × 6 novelties, 2 trials per cell) to probe
   the interaction surface and recover the novelty exponent γ.
3. Standard density, novelty, and low-density blocks retained from v6.

The rest of the pipeline—hierarchical priors, Ex-Gaussian likelihood,
non-centred parameterisation, diagnostics, and recovery statistics—remains
unchanged from v6.
...
class TICParameterRecoveryV7:
    ...
    def __init__(
        ...
        # Hyper-priors (tightened to metabolically plausible ranges)
        self.hyper_means = {
            "D0": 0.13,
            "lambda": np.log(1.0),
            "kappa": np.log(0.25),
        ...
        # Bookend design parameters
        self.anchor_density = 0.05
        self.anchor_novelty_mu = 0.05
        self.anchor_novelty_sd = 0.02
        self.n_trials_anchor = 15
        ...
        self.novelty_levels = np.array([0.1, 0.2, 0.4, 0.6, 0.8])
        self.n_trials_per_novelty = 6
        self.fixed_density_block_b = 0.25

        # Factorial cross block (densities x novelties x 2 trials)
        self.factorial_densities = np.array([0.2, 0.5, 0.8])
        self.factorial_novelties = np.array([0.1, 0.3, 0.5, 0.7, 0.8, 0.9])
        self.n_trials_per_factorial_cell = 2
    ...
    def generate_synthetic_data(self) -> Tuple[List[ParticipantData], Dict[str, np.ndarray]]:
        ...
        true_sigma_D0 = self.rng.lognormal(np.log(0.03), 0.3)

        true_mu_lambda = self.rng.normal(self.hyper_means["lambda"], self.hyper_scales["lambda"])
        true_sigma_lambda = self.rng.lognormal(np.log(0.25), 0.3)

        true_mu_kappa = self.rng.normal(self.hyper_means["kappa"], self.hyper_scales["kappa"])
        true_sigma_kappa = self.rng.lognormal(np.log(0.30), 0.3)

        true_mu_gamma = np.clip(self.rng.normal(self.hyper_means["gamma"], 0.15), 0.50, 0.95)
        true_sigma_gamma = self.rng.lognormal(np.log(0.20), 0.3)
        ...
        for pid in range(nP):
            ...
            # Low-entropy anchor block (zero lapses, reduced noise)
            D_eff_anchor = self.effective_density_map(self.anchor_density)
            anchor_gaussian = self.gaussian_sd * 0.5
            anchor_tau = self.exponential_tau * 0.5

            for _ in range(self.n_trials_anchor):
                Phi_prime = float(np.clip(phi_trait[pid] + self.rng.normal(0.0, 0.08), 0.2, 2.0))
                N_true = float(np.clip(self.rng.normal(self.anchor_novelty_mu, self.anchor_novelty_sd), 0.01, 0.20))
                N_obs = float(np.clip(N_true + self.rng.normal(0.0, 0.03), 0.01, 0.25))

                numerator = 1.0 + kap * (N_true ** gam)
                denom = lam * (D0_i + D_eff_anchor) * Phi_prime
                T_s_true = self.T_o * numerator / max(denom, 1e-6)

                noise = self.rng.normal(0.0, anchor_gaussian) + self.rng.exponential(anchor_tau)
    ...
    def save_diagnostics(
        ...
        idata: az.InferenceData,
        sim_idx: int,
        output_dir: str = "simulations/diagnostics_v7",
    ) -> None:
    ...
    def run(self, chains: int = 4) -> None:
        print("Running TIC Parameter Recovery v7 (Hierarchical Bayesian + Bookend Design)")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Hierarchical TIC parameter recovery (v7)",
        epilog="Recommended validation: --n-sim 200 --n-participants 35 --chains 4",
    )
    parser.add_argument("--n-sim", type=int, default=5, help="Number of simulations (default: 5)")
    parser.add_argument("--n-participants", type=int, default=18, help="Participants per simulation (default: 18)")
...
def main() -> None:
    args = parse_args()
    recovery = TICParameterRecoveryV7(
