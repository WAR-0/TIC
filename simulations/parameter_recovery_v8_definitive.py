#!/usr/bin/env python3
"""
TIC Parameter Recovery Simulation v8 (Definitive Validation)

- Hierarchical PyMC/NUTS model (unchanged core from v6).
- Evidence-based "Bookend" design only: low-entropy anchor + high-power factorial block.
- Empirically grounded Ex-Gaussian noise (≈20% CV overall).
- Weakly informative priors reflecting dossier consensus.

Default CLI values run a quick smoke test. Use
  python simulations/parameter_recovery_v8_definitive.py --n-sim 200 --n-participants 35
for the full validation batch used in the manuscript.
"""
...
class TICParameterRecoveryV8:
    """Hierarchical Bayesian parameter recovery for the TIC model (final design)."""
    ...
    def __init__(
        ...
        # Constant reference duration (seconds)
        self.T_o = 60.0

        # Hyper-prior centres (evidence-based) + participant bounds
        self.hyper_means = {
            "D0": 0.15,
            "lambda": np.log(1.0),
            "kappa": np.log(0.2),
    ...
    def generate_synthetic_data(self) -> Tuple[List[ParticipantData], Dict[str, np.ndarray]]:
        ...
        # Draw true population hyper-parameters (weakly informative)
        ...
        for pid in range(nP):
            ...
            # Anchor block (use full noise model but zero lapses)
            ...
            for _ in range(self.n_trials_anchor):
                ...
                N_obs = float(np.clip(N_true + self.rng.normal(0.0, 0.04), 0.01, 0.25))
            ...
            # Factorial block (3×5×20 = 300 trials)
    ...
    def build_model(self, participants: List[ParticipantData]) -> pm.Model:
        ...
        with pm.Model() as model:
            ...
            pm.ExGaussian(
                "Ts_obs", mu=mu_preds, sigma=sigma_obs, nu=tau_obs, observed=Ts
            )
    ...
    def save_diagnostics(self, idata: az.InferenceData, sim_idx: int, output_dir: str = "simulations/diagnostics_v8") -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Hierarchical TIC parameter recovery (v8 definitive design)",
        epilog="Full validation: --n-sim 200 --n-participants 35 --draws 1500 --tune 1500",
    )
...
def main() -> None:
    args = parse_args()
    recovery = TICParameterRecoveryV8(
